...
FNoticeManager::FNoticeManager()
{
    ...
    const char *sql_createtable = "CREATE TABLE IF NOT EXISTS notice(id int auto_increment, sid int, title VARCHAR(256), readed int, start_time int, end_time int);";
	sqlite3_stmt *stmt;
    ...
    const char *sql_stmt_add = "INSERT INTO notice (sid, title, readed, start_time, end_time) VALUES (?,?,?,?,?);";
    ...
    const char* sql_stmt_get =" SELECT sid, title, readed ,start_time, end_time FROM notice ORDER BY start_time DESC;";
...
bool FNoticeManager::addNotice(int sid, int type, std::string &title, time_t start, time_t end)
{
    ...
    ok |= sqlite3_bind_text(_sqlite_stmt_add, 4, crossapp_format_string("%d", start).c_str(), -1, SQLITE_TRANSIENT);
    ok |= sqlite3_bind_text(_sqlite_stmt_add, 4, crossapp_format_string("%d", end).c_str(), -1, SQLITE_TRANSIENT);
...
std::vector<FNotice>& FNoticeManager::getNotices()
{
    ...
    while(sqlite3_step(_sqlite_stmt_get) == SQLITE_ROW)
    {
        ...
        notice.startTime = sqlite3_column_int(_sqlite_stmt_get, 3);
        notice.endTime = sqlite3_column_int(_sqlite_stmt_get, 4);
