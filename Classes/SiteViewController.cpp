...
void SiteViewController::viewDidLoad()
{
	...
	tabBg->setFrame(DRect(_px(0), _px(0), m_winSize.width, _px(150)));
	...
	button = CAButton::createWithFrame(DRect(m_winSize.width - 100, _px(50), _px(50), _px(50)), CAButtonTypeRoundedRect);
...
void SiteViewController::showAlert()
{
	...
	m_alertView = CAView::createWithFrame(DRect(0, 0, m_winSize.width, m_winSize.height - _px(150)));
	this->getView()->addSubview(m_alertView);

	CAImageView* bg = CAImageView::createWithFrame(DRect(0, 0, m_winSize.width, m_winSize.height - _px(150)));
